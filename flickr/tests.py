...
class FlickrModelTests(TestCase):
    ...
    def setUp(self):
        self.client = Client()
        self.flickr_user = FlickrAccount.objects.create()
        self.flickr_user2 = FlickrAccount.objects.create()


    def test_user(self):
        FlickrAccount.objects.update_from_json(self.flickr_user.id, json_user)
        fu = FlickrAccount.objects.get(flickr_id=json_user['person']['id'])
    ...
    def test_photo_create(self):
        photo = Photo.objects.create_from_json(flickr_account=self.flickr_user, info=json_info, sizes=json_sizes, exif=json_exif)
    ...
    def test_photoset(self):
        ...
        photoset = PhotoSet.objects.create_from_json(flickr_account=self.flickr_user, info=json_set_info['photoset'], photos=json_set_photos)
